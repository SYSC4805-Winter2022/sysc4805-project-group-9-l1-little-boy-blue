...
class Robot:
    ...
    def start(self):
        ...
        while True:  # Main loop code, can be time dependent
            ...
            if middleDetectionState:
                print(inspect.stack()[0][3], "Object detected")
                self.avoid_object()
    ...
    def avoid_object(self):
        """
            Avoid an object
        """
        print(inspect.stack()[0][3], "prev_dir", self.prevDir)
        print(inspect.stack()[0][3], "curr_dir", self.currDir)
        init_position = self.get_position()
        is_detected = True
        prev_dir = self.prevDir
        current_dir = self.currDir
        leftDetectedState, middleDetectionState, rightDetectedState, leftDetectedPoint, middleDetectedPoint, rightDetectedPoint = self.get_object_detection_sensor_data()
        if leftDetectedState:
            self.turn_right(90)
            self.prevDir = self.currDir
            if self.currDir == Direction.North:
                ...
                self.currDir = Direction.South
            elif self.currDir == Direction.South:
                self.currDir = Direction.West
            elif self.currDir == Direction.West:
                self.currDir = Direction.North
        elif rightDetectedState:
            self.turn_left(90)
            self.prevDir = self.currDir
            if self.currDir == Direction.North:
                self.currDir = Direction.West
            ...
            if self.currDir == Direction.North or self.currDir == Direction.South:
                ...
                self.prevDir = self.currDir
                self.currDir = Direction.West
            elif self.currDir == Direction.West or self.currDir == Direction.East:
                self.turn_to(self.prevDir.value)
                tmp = self.currDir
                self.currDir = self.prevDir
                self.prevDir = tmp
        while is_detected:
            self.move_straight(self.MOVE_STRAIGHT_SPEED)
            leftDetectedState, middleDetectionState, rightDetectedState, leftDetectedPoint, middleDetectedPoint, rightDetectedPoint = self.get_object_detection_sensor_data()
            if middleDetectionState and 0 < middleDetectedPoint[2] < 0.5:
                print(inspect.stack()[0][3], "Object detected")
                self.avoid_object()
        ...
        # self.turn_to(current_dir.value)
        # self.move_straight(self.MOVE_STRAIGHT_SPEED)
        ...
        self.navigate_back(init_position, avoided_position, current_dir)
        print(inspect.stack()[1][3], "done")
    ...
    def navigate_back(self, initial_pos, avoided_pos, prev_dir):
        ...
        print("curr_dir", self.currDir)

        if self.currDir == Direction.North:
            # case West with left obstacle and East with right obstacle
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            self.move_to(x=self.AVOID_OBJECT_DISPLACEMENT)
            left_back_detection_state, _, right_back_detection_state, _ = self.get_laser_data()
            leftDetectedState, middleDetectionState, rightDetectedState, leftDetectedPoint, middleDetectedPoint, rightDetectedPoint = self.get_object_detection_sensor_data()
            if prev_dir == Direction.West:
                expression = lambda: left_back_detection_state or leftDetectedState
            elif prev_dir == Direction.East:
                expression = lambda: right_back_detection_state or rightDetectedState
            self.move_straight(self.MOVE_STRAIGHT_SPEED)
            while expression():
                left_back_detection_state, _, right_back_detection_state, _ = self.get_laser_data()
                leftDetectedState, middleDetectionState, rightDetectedState, leftDetectedPoint, middleDetectedPoint, rightDetectedPoint = self.get_object_detection_sensor_data()
            ...
            self.prevDir = self.currDir
            self.currDir = Direction.South
            self.move_to(y=avoided_pos[1] - initial_pos[1])
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            return
        elif self.currDir == Direction.South:
            # case West with right obstacle, East with left obstacle, West/East with no obstacle
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            self.move_to(x=self.AVOID_OBJECT_DISPLACEMENT)
            ...
            self.prevDir = self.currDir
            self.currDir = Direction.North
            self.move_to(y=initial_pos[1]-avoided_pos[1])
            # back to nav point
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            ...
            # case North with left obstacle, South with right obstacle
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            ...
            self.prevDir = self.currDir
            self.currDir = Direction.West
            self.move_to(x=avoided_pos[0] - initial_pos[0])
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            return
        elif self.currDir == Direction.West:
            # case North with right obstacle, South with left obstacle, North/South with no obstacle
            self.turn_to(prev_dir.value)
            self.prevDir = self.currDir
            self.currDir = prev_dir
            self.move_to(y=self.AVOID_OBJECT_DISPLACEMENT)
